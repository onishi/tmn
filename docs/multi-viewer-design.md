...
現在の `signaling/src/room.ts` は「役割(`caster`/`viewer`)」単位でメッセージを中継しており、
...
- Caster発のoffer/ice-candidateにも `viewerId` を含めることを必須にし、Roomは該当Viewerのsocketのみに転送する(現在の「role全員に送る」実装から「特定のviewerId宛てに送る」実装へ変更)
- Viewer発のanswer/ice-candidateは自身の `viewerId` を含めて送信し、Caster側で対応するPeerConnectionを引くのに使う
...
- 後方互換は不要(Viewer・Casterとも同一リポジトリ内で同時にプロトコルを更新するため)
