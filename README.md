...
```
[配信スマホ(Androidネイティブアプリ)]
      │  WebRTC(STUN: stun.cloudflare.com)
      ▼
[Cloudflare Realtime TURN](NAT越えが必要な時だけ経由)
      │
      ▼
[視聴Webアプリ] ← Cloudflare Pagesでホスティング
      ▲
      │  シグナリング(接続情報交換・視聴リクエスト通知)
[Cloudflare Workers + Durable Objects]
```

全構成要素をCloudflareに統合することで、契約先・管理画面を1つにまとめられる。配信端末はAndroidを採用し、配信アプリはネイティブ(Kotlin)で実装する(フォアグラウンドサービスにより画面消灯状態でも安定して配信を継続できるため。PWAは開発は手軽だが、バックグラウンド動作の制約が大きく監視カメラ用途には不向き)。

### 1.2 コンポーネント

| コンポーネント | 役割 | 技術 |
|---|---|---|
| 配信アプリ | カメラ映像取得・配信 | Androidネイティブアプリ (Kotlin, CameraX, WebRTC Android SDK, Foreground Service) |
| 視聴アプリ | 映像受信・表示 | 静的Webページ、Cloudflare Pagesでホスティング |
...
**含む:**
- カメラ映像の一方向配信(配信スマホ → 視聴側)
- 1対1接続(視聴者1人を想定)
- 配信アプリはAndroid端末にインストールし、Foreground Serviceとして常時起動
...
2. シグナリング用Workerの実装(Durable Objectsでルームごとの接続情報をWebSocket経由で中継。視聴側の接続をトリガーに配信側へ「配信開始」通知を送る仕組みを含む)
3. 配信側Androidアプリ実装(Android Studio, Kotlin。シグナリングWorkerとのWebSocket接続を常時維持し、視聴リクエスト受信時にCameraXでカメラ取得 → WebRTC Android SDKでRTCPeerConnection作成 → Offer生成、という流れをオンデマンドで実行。Foreground Serviceとして起動し画面消灯・バックグラウンドでも待機・配信を継続)
4. 視聴側Webアプリ実装(シグナリングWorkerに接続要求を送信 → Answer取得 → 映像受信・表示)
...
6. Androidアプリをビルド・署名し、配信用スマホにインストール。常時電源に接続した状態で設置し、端末のスリープ設定・バッテリー最適化除外も設定
7. 視聴側をCloudflare Pagesにデプロイ
...
- **動体検知・通知**:配信側でフレーム差分を検出しPush通知(Firebase Cloud Messaging)。動体検知時のみ配信を自動開始する運用にすれば、オンデマンド配信方針とも自然に統合できる
...
- **配信アプリのiOS対応検討**:必要であればiOS版もKotlin Multiplatform等で追加開発
...
- **配信アプリ**: Kotlin, CameraX, WebRTC Android SDK, Foreground Service, WorkManager(再接続制御)
- **視聴アプリ**: WebRTC(ブラウザ標準API)
